            ...
            for k in range_k:
                ...
                if propagation_enabled:
                    ...
                    if 0 <= i + direction < source_shape[0] and 0 <= j + direction < source_shape[1]:
                        if (new_f[i + direction, j,k][2] + i >= target_patches.shape[0]) or (
                                        new_f[i + direction, j, k][3] + j >= target_patches.shape[1]):
                        ...
                        if new_f[i, j + direction, k][2] + i >= target_patches.shape[0] or (
                                        new_f[i, j + direction, k][3] + j >= target_patches.shape[1]):
    ...
    for i in range_y:
        ...
        for j in range_x:
            ...
            for m in range(k):
                ...
                # /// if you have time figure out how to properly use the _tiebreaker object.
...
def reconstruct_source_from_target(target, f):
    ...
    axis_y = (coor_y + nnf_y).astype(np.int32)
    axis_x = (coor_x + nnf_x).astype(np.int32)
    axis_y =np.clip(axis_y, - tar_shape[0] + 1, tar_shape[0] - 1)
    axis_x = np.clip(axis_x, - tar_shape[1] + 1, tar_shape[1] - 1)
